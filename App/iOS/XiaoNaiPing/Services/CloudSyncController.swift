...
final class CloudSyncController: ObservableObject {
    ...
    var isNativeWeChatLoginConfigured: Bool {
        CloudSyncConfiguration.isWeChatLoginConfigured
    }
    ...
    func requestPhoneCode(phoneNumber: String) async {
        await perform("正在发送验证码", "正在请求手机号登录验证码。") {
            ...
            guard Self.validateE164PhoneNumber(normalizedPhoneNumber) else {
                throw CloudSyncError.invalidPhoneNumber
            }
    ...
    func verifyPhoneCode(phoneNumber: String, code: String, store: BabyRecordStore) async {
        await perform("正在登录", "正在验证手机号验证码。") {
            ...
            guard Self.validateSmsCode(normalizedCode) else {
                throw CloudSyncError.invalidVerificationCode
            }
    ...
    func loginWithWeChat(store: BabyRecordStore) async {
        await perform("正在登录微信", "正在通过微信授权登录。") {
            guard isNativeWeChatLoginAvailable else {
                throw CloudSyncError.server("当前设备无法使用微信授权，请改用手机号登录。")
            }
    ...
    func deleteCloudPhoto(_ photo: BabyPhoto) async -> Bool {
        ...
        do {
            let client = try makeClient()
            guard let session = sessionStore.session else { throw CloudSyncError.missingSession }
    ...
    private func restoreRemoteData(store: BabyRecordStore, client: CloudSyncAPIClient, token: String) async throws {
    ...
    private func uploadEverything(store: BabyRecordStore, client: CloudSyncAPIClient, token: String) async throws {
        ...
        for asset in store.localPhotoSyncAssets() {
    ...
    private func makeClient() throws -> CloudSyncAPIClient {
        guard let baseURL = CloudSyncConfiguration.apiBaseURL else {
            throw CloudSyncError.missingBaseURL
        }
