...
7. 当前最小后端和 iOS 云备份入口已实现，但未部署到华为云生产环境。
...
- [ ] 核心流程可跑通。
- [x] 隐私文案草案完整。
- [x] 审核登录路径草案准备完成：资料 -> 账号与备份 -> 创建账号并备份。
- [x] 生产备份恢复测试环境和生产环境边界清楚，证据见 `Backend/proof/remote-api.json`。
...
- [ ] 宝塔 MySQL migration 已通过；数据库备份和恢复演练待补。
...
- [x] 5 张 iPhone 模拟器实现截图证据，包括账号与备份登录面板。
...
- [x] Review Notes 预检报告，当前结论已通过；审核说明覆盖免费、无广告、无医疗建议、账号方式、照片原图备份、删除路径、疫苗边界和不依赖 debug code。
