...
- 阶段：API 设计与最小实现更新
- 日期：2026-06-14
- 当前结论：已新增最小第一方 API 实现 `Backend/api/server.py`，覆盖手机号验证码登录、微信登录、登录后自动云同步、照片原图上传/下载/删除、账号删除和第一方合规埋点；服务端已支持短信 webhook、阿里云短信 webhook adapter、微信 code 换 openid/unionid 和白名单行为事件，生产域名、真实阿里云短信签名/模板、微信开放平台凭证、iOS OpenSDK、云主机和对象存储区域仍需发布前私有确认
...
| `GET /v1/family` | 获取当前账号的私密家庭成员资格 | 已实现 |
| `POST /v1/family` | 创建私密家庭组并获取邀请码 | 已实现 |
| `POST /v1/family/join` | 用邀请码加入私密家庭组 | 已实现 |
| `POST /v1/family/invite/rotate` | 创建者使旧邀请码失效并生成新邀请码 | 已实现 |
| `DELETE /v1/family` | 当前成员退出家庭；创建者退出时转交家庭 | 已实现 |
| `DELETE /v1/family/members/{accountId}` | 创建者移除成员并立即撤销其共享记录访问权 | 已实现 |
| `PUT /v1/family/records` | 批量上传家庭组内允许类型的记录信封 | 已实现 |
| `GET /v1/family/records?since=` | 按游标增量拉取家庭共享记录 | 已实现 |
...
3. 不接受宝宝昵称、生日、照片、对象 key、手机号、微信 openid/unionid、登录凭证、token、定位、User-Agent 或任意自由文本。
...
第一版账号入口为手机号验证码和微信登录；认证成功后自动开启云同步：

1. 手机号验证码校验成功后，服务端查找或创建对应账号并签发短期 `sessionToken`。
2. 微信授权 code 校验成功后，服务端查找或创建对应账号并签发短期 `sessionToken`。
3. 客户端把 `sessionToken` 存入 Keychain，并立即将本地数据加入该账号的自动云同步队列。
...
6. 家庭共享只允许已加入同一家庭组的账号访问，且不得传输照片原图、照片元数据、提醒偏好、账号凭证或身份标识；创建者可轮换邀请码、移除成员，所有成员可退出，撤销后服务端立即拒绝其继续读写共享记录。
