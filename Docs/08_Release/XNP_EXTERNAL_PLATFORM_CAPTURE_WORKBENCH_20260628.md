...
结论：这份工作台用于现场采集微信开放平台、短信服务商、OBS、备案、隐私标签和生产 proof。它不是提交许可，也不代表这些外部平台已经配置完成；只有跨 App 总守卫 `canSubmit=true` 后才进入 App Store Connect 提交审核。

来源文件：

- `Docs/08_Release/XNP_EXTERNAL_PLATFORM_CAPTURE_PACKET_20260628.json`
- `Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260628.md`
- `Docs/08_Release/APP_STORE_CONNECT_COPY_PASTE_20260628.md`
- `Docs/08_Release/APP_STORE_PRIVACY_LABEL.json`
- `Docs/08_Release/AppStoreEvidence/CAPTURE_GUIDE.md`
- `Docs/08_Release/AppStoreEvidence/RealDevice/EXECUTION_SHEET_20260628.md`
- `Backend/proof/production-readiness-20260628T-current.json`
...
- 结果文件必须记录 `canSubmitAtCapture`、`redactionReviewed`、当前 `cross-app-submission-readiness` proof、各平台截图路径、复跑 proof 和真机联动状态；它只是同轮证据索引，不能替代截图/PDF、真实短信实发、生产 proof、真机回归或 `canSubmit=true`。

## 1.1 同轮一致性和哈希清单

从模板复制出的 `EXTERNAL-PLATFORM-CAPTURE-RESULT.json` 必须填写 `sameRoundEvidenceManifest`，并至少记录：

- `captureRoundId`，例如 `xnp-external-platforms-2026-06-28`。
- `SHA-256` / `sha256` 哈希：每个最终证据文件、短信实发 proof、微信配置 proof、AASA proof、OBS proof 和生产 proof 都必须有哈希。
- `Docs/08_Release/XNP_EXTERNAL_PLATFORM_CAPTURE_PACKET_20260628.json`
- `Docs/08_Release/SMS_PROVIDER_LIVE_SEND_PACKET_20260628.json`
- `Docs/08_Release/WECHAT_RELEASE_CONFIGURATION_PACKET_20260628.json`
- `Docs/08_Release/OBS_STORAGE_PROOF_PACKET_20260628.json`
- `Docs/08_Release/PRODUCTION_PROOF_REFRESH_PACKET_20260628.json`
- `Backend/proof/auth-providers-sms-live-20260628T-current.json`
- `Backend/proof/wechat-client-configuration-20260628T-current.json`
- `Backend/proof/universal-links-20260628T-current.json`
- `Backend/proof/storage-backend-20260628T-current.json`
- `Backend/proof/production-readiness-20260628T-current.json`

只有采集时间、文件日期、SHA-256 和复跑 proof 都属于同一轮采集，才能把 `allDependenciesCurrentAndPassed` 改为 true。不能跨天拼接，不能把 2026-06-27 的截图、微信临时图、短信旧发送记录、旧 OBS proof 或旧生产 proof 混入 2026-06-28 结果文件。
...
python3 Backend/scripts/check_universal_links.py \
  --output Backend/proof/universal-links-20260628T-current.json
...
通过口径：短信 provider 服务器 proof 只能证明后端配置存在，不能替代服务商截图和真实短信实发。真实实发 proof 必须单独保存为 `Backend/proof/auth-providers-sms-live-20260628T-current.json`，并且只有它和 `Backend/proof/auth-providers-20260628T-current.json` 都通过后，才能把 sms-live proof 同步到稳定 alias `Backend/proof/auth-providers.json`。
...
- 入库后必须复跑 `check_app_store_evidence.py`、`check_production_readiness.py` 和跨 App `check-cross-app-submit-ready`；只复制图片不算完成。
...
python3 Backend/scripts/check_production_readiness.py \
  ...
  --auth-providers-proof Backend/proof/auth-providers-20260628T-current.json \
...
最后回到 `/Users/smianmian/Emotion Isle` 跑：
