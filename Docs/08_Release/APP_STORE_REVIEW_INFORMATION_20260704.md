...
结构化审核测试账号执行包：`Docs/08_Release/APP_REVIEW_TEST_ACCOUNT_PACKET_20260704.json`。该包状态为 `review-test-account-packet-not-evidence`；只约束 App Review 私密 Sign-In Information 的填写、脱敏证据、RD-10/RD-13/RD-14/RD-15 真机采集、账号删除和复跑 gate，不保存恢复密钥，不能作为提交许可。
...
App Review 测试使用恢复密钥测试账号。真实值只从本机 ignored 文件 `.env.xnp-review-account` 读取，只能填入 App Store Connect 的 App Review Information 私密字段。
...
1. 打开 App -> 登录小奶瓶 -> 恢复密钥登录。若已进入主界面，也可以在“资料 -> 账号与同步”中使用恢复密钥登录。
...
6. 微信登录不是审核前置条件；只有设备已安装微信且原生微信授权可用时才会显示微信入口。审核员无需安装微信，请优先使用恢复密钥测试账号。
7. 灵动岛、锁屏 Live Activity 和小组件仅展示用户设置或用户主动记录的状态，不生成健康建议、压力提醒、喂养建议或医疗判断。若已设置固定喝奶间隔，新增喂养时的顺延选项为不顺延或 +5、+10、+15、+20、+25、+30 分钟；下一次提醒按本顿结束时间 + 固定间隔 + 顺延分钟重排，本顿无喂养时长时按本顿发生时间计算。
```
...
| 中国大陆 / App Store 人工项 | `Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md`：`03-app-filing`、`04-privacy-label`、`17-age-rating-result` |
