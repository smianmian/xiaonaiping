...
- 项目：小奶瓶 / 宝宝成长记录
- 阶段：全球 App Store Connect 元数据草案同步
- 日期：2026-07-04
- 公司主体：深圳市闪现生活科技有限公司
...
10. 当前可填表版本以本文件的多语言草案与审核说明为准；旧日期填表快照已移除，提交前需按现行手机号/微信认证方案重建现场填写证据。
...
3. 全球首发同轮提供简体中文与繁体中文资源，并按目标地区完成商店本地化复核。
4. 疫苗提醒文案必须避免医疗建议。

## 待我确认的问题

1. App Store Connect 创建后，最终 App 名称、副标题、分类和截图仍需人工核对一次。
2. 全球首发所需的简体中文、繁体中文元数据和审核截图是否已最终校对。
3. 各目标地区的商店本地化与高频文案是否已人工校对。
...
3. 不提供医疗诊断。
4. 数据本地优先，同时支持账号同步恢复。
5. 账号支持手机号验证码和微信授权登录，不采集邮箱。
6. 提交前必须补齐真实短信服务、微信开放平台、实发验证和真机回归证据。
7. 用户主动加入 App 的宝宝照片原图会用于私有云同步。
8. 审核可在资料页 -> 账号与同步中使用手机号验证码或微信授权登录；登录后自动同步。
...
| 材料 | 路径 |
|---|---|
| App Store 元数据与审核说明 | `Docs/08_Release/APP_STORE_METADATA.md` |
| 全球首发与地区合规策略 | `Docs/08_Release/REGIONAL_LAUNCH_STRATEGY.md` |
...
| 审核证据采集规则 | `Docs/08_Release/AppStoreEvidence/README.md` |
| D-U-N-S 后续动作 | `Docs/08_Release/APPLE_DEVELOPER_DUNS_HANDOFF.md` |

## 当前不可提交原因

旧认证方案生成的发布 proof 已移除。当前 App Store Connect 文案可以先建草稿，但在按手机号验证码/微信授权与自动云同步方案重新生成上线总审计和生产就绪证据前，不能提交审核：
...
4. 未归档 App Store Connect 公司主体、全球可售地区、隐私标签、适用备案、签名归档和 TestFlight 人工证据。
...
| 描述 | 小奶瓶是一款为新手父母和照护者设计的宝宝成长记录 App。你可以快速记录喂养、睡眠、排便、身高体重、疫苗提醒和成长瞬间，手动设置下一次喝奶提醒，也可以在新增喂养后按 5 分钟一档手动顺延下一次提醒，把宝宝照片整理在私密时间线里。数据默认本地优先保存；开启账号与同步后，用户可通过手机号验证码或微信授权登录私有账号，登录后 App 会自动同步宝宝记录、照片元数据和用户主动加入 App 的照片原图，也可手动立即同步，便于换机恢复。小奶瓶不根据奶量、月龄、传感器或健康数据自动推算喂养时间，不提供医疗诊断或治疗建议，疫苗模板仅用于记录和提醒，实际接种安排请以医生和当地官方信息为准。 |
| 新版本说明 | 第一版：宝宝档案、日常记录、喝奶提醒与手动顺延、成长记录、疫苗提醒、照片时间线、手机号验证码或微信登录、自动云同步恢复和云端账号删除。 |
...
| 描述 | 小奶瓶是一款為新手父母和照護者設計的寶寶成長記錄 App。你可以快速記錄餵養、睡眠、排便、身高體重、疫苗提醒和成長瞬間，手動設定下一次喝奶提醒，也可以在新增餵養後按 5 分鐘一檔手動順延下一次提醒，把寶寶照片整理在私密時間線裡。資料預設本機優先保存；開啟帳號與同步後，使用者可透過手機號碼驗證碼或微信授權登入私有帳號，登入後 App 會自動同步寶寶記錄、照片中繼資料和使用者主動加入 App 的照片原圖，也可手動立即同步，方便換機還原。小奶瓶不會根據奶量、月齡、感測器或健康資料自動推算餵養時間，不提供醫療診斷或治療建議，疫苗模板僅用於記錄和提醒，實際接種安排請以醫生和當地官方資訊為準。 |
| 新版本說明 | 第一版：寶寶檔案、日常記錄、喝奶提醒、成長記錄、疫苗提醒、照片時間線、手機號碼驗證碼或微信登入、自動雲端同步還原和雲端帳號刪除。 |
...
| Description | Xiao Nai Ping is a gentle baby journal for parents and caregivers. Record feeding, sleep, diapers, height, weight, vaccine reminders, feeding reminders, and memorable photos in a low-pressure daily flow. You can set the next feeding reminder and manually defer it in 5-minute steps after adding a feeding. Your data is local-first. After signing in with SMS verification or WeChat authorization, the app automatically syncs baby records, photo metadata, and original photos you explicitly add to the app; you can also sync immediately, making device restore easier. Xiao Nai Ping does not infer feeding times from volume, age, sensors, or health data, and does not provide medical diagnosis, treatment advice, or professional vaccine guidance. Vaccine templates are only for record keeping and reminders; please follow your doctor and local public health guidance. |
| What’s New | First release: baby profile, daily logs, feeding reminders, growth records, vaccine reminders, photo timeline, SMS or WeChat sign-in, automatic cloud sync and restore, and cloud account deletion. |
...
数据默认本地优先保存。用户可以在“资料 -> 账号与同步”中使用手机号验证码或微信授权登录。登录后 App 会自动同步宝宝记录、照片元数据，以及用户主动加入 App 的照片原图，用户也可以手动立即同步。服务端保存哈希后的账号标识，不采集邮箱。
...
当前仓库不保存审核登录凭据。若提交包需要审核登录，必须在提交前基于届时实际可用的生产手机号验证码或微信授权流程填写 App Review Information 私密字段，并完成同构建真机验证；正式提交包不得提供或依赖 debug code，审核登录必须与真实用户生产认证流程一致。
