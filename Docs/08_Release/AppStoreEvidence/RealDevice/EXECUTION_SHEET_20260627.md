...
状态：现场拍摄和填表用，不是已完成证据。正式提交仍以 `Docs/08_Release/AppStoreEvidence/12-real-device-regression.md`、`Backend/proof/testflight-regression-plan.json` 和 `Backend/proof/app-store-evidence-20260627T-current.json` 为准。
...
| RD-15 删除云端账号与同步 / 账号删除 | `RealDevice/RD-15-account-delete.png` | 删除后旧 token 失效，云端同步和照片对象删除 |
...
| RD-23 锁屏/桌面小组件 / 桌面/锁屏小组件 | `RealDevice/RD-23-lock-screen-notification-stack.png`、`RealDevice/RD-23-home-widget-summary.png` | 锁屏通知栈不遮挡；桌面小组件只读展示今日摘要，不展示备注、真实照片、token 或对象 key |
...
| RD-23 锁屏/小组件 | 锁屏通知栈遮挡，或小组件裁剪/展示隐私照片、备注、token、对象存储 key | RealDevice/RD-23-widget-fail.png | RealDevice/RD-23-widget-retest.png | 待填 | 未通过前不得提交 App Store Connect |
...
```bash
python3 Backend/scripts/check_app_store_evidence.py --output Backend/proof/app-store-evidence-20260627T-current.json
...
python3 Backend/scripts/check_production_readiness.py --output Backend/proof/production-readiness-20260627T-current.json --allow-incomplete
```
