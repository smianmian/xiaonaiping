...
- `Docs/08_Release/XNP_EXTERNAL_PLATFORM_CAPTURE_WORKBENCH_20260704.md`
...
- `Docs/08_Release/PRODUCTION_PROOF_REFRESH_STATUS_20260704.json`
...
当前生产 proof 状态快照：`Docs/08_Release/PRODUCTION_PROOF_REFRESH_STATUS_20260704.json`。该文件由 `python3 Backend/scripts/check_production_proof_refresh_status.py --date 2026-07-04 --allow-incomplete --output Docs/08_Release/PRODUCTION_PROOF_REFRESH_STATUS_20260704.json` 按当前本机 proof 文件状态生成，状态为 `current-proof-status-not-submit-permission`，当前 `stableAliasSyncAllowed=false`；`deploymentProofCurrent` 已存在但仍记录 OBS、短信和微信生产配置 blocker，`production-readiness-20260704T-current.json` 仍要求补当天 `/internal` 阻断证明，`missingProofs` 仅剩 `authProvidersSmsLiveCurrent`，`failedProofs` 包含 deployment proof blockers、auth provider、App Store evidence、production readiness、launch objective audit 和相关 stable alias；`secretScanFailures=0`。结论保持：current proof files are incomplete or failed; do not sync stable aliases。
...
- 必须交叉引用当天 `Backend/proof/auth-providers-sms-live-20260704T-current.json`、`Backend/proof/wechat-client-configuration-20260704T-current.json`、`Backend/proof/storage-backend-20260704T-current.json` 和 `Backend/proof/production-readiness-20260704T-current.json`。
...
| `phoneLoginProviderConfigured` | `auth-providers-20260704T-current.json`、短信服务商截图、真实实发 proof | webhook provider、服务商模板、发送成功和 iOS 26.5 登录回归都通过 | 不展示完整手机号、验证码或 `XNP_SMS_SECRET` |
...
python3 Backend/scripts/check_public_pages.py \
  --base-url https://api.mewpow.com/xiaonaiping \
  --output Backend/proof/public-pages-20260704T-current.json
python3 Backend/scripts/check_legal_drafts.py \
  --output Backend/proof/legal-drafts-20260704T-current.json
```
...
- Tracking 为否。
- 年龄分级结果与 `Docs/08_Release/APP_STORE_AGE_RATING_ANSWERS_20260704.md` 一致。
...
python3 Backend/scripts/check_mainland_filing_materials.py \
  --output Backend/proof/mainland-filing-materials-20260704T-current.json
python3 Backend/scripts/check_app_store_connect_evidence_materials.py \
  --output Backend/proof/app-store-connect-evidence-materials-20260704-current.json
```
...
6. 最后跑 iOS 26.5 Release 包检查、App Store evidence、production readiness、launch objective audit、TestFlight regression、provider evidence、mainland filing、signed archive/TestFlight materials 和 `PRODUCTION_PROOF_REFRESH_STATUS_20260704.json`。
...
- 不把 `auth-providers-20260704T-current.json` 的 provider 配置通过，写成短信服务商截图、真实短信实发或 iOS 26.5 真机登录已通过。
...
python3 Backend/scripts/check_production_readiness.py \
  ...
  --auth-providers-proof Backend/proof/auth-providers-20260704T-current.json \
