...
版本页、发布方式、出口合规、IDFA 和内容权利填写见 `Docs/08_Release/APP_STORE_VERSION_RELEASE_SETTINGS_20260704.md`。

结构化草稿总览见 `Docs/08_Release/APP_STORE_CONNECT_DRAFT_20260704.json`，该 JSON 只用于核对草稿字段，不代表可提交审核。

人工填写当天执行包见 `Docs/08_Release/APP_STORE_CONNECT_ENTRY_SESSION_PACKET_20260704.json`，该包只约束填写顺序、停机条件、页面证据和复跑命令，不代表 App Store Connect 已填写或可提交审核。

最终人工粘贴、同轮 build / 证据截图核对见 `Docs/08_Release/APP_STORE_CONNECT_FINAL_ENTRY_AUDIT_20260704.md`。
...
以 `Docs/08_Release/APP_STORE_PRIVACY_LABEL.json` 为最终源文件。人工逐项核对表见 `Docs/08_Release/APP_STORE_PRIVACY_ANSWERS_20260704.md`。当前摘要：
...
审核测试登录请优先使用 App Review Information 中提供的恢复密钥测试账号。手机号和微信测试号会在真实短信服务与微信开放平台配置完成后补充；正式提交包不提供、不依赖 debug code。微信登录不是审核前置条件；只有设备已安装微信且原生微信授权可用时才会显示微信入口，审核员无需安装微信。
```
...
- `Backend/proof/auth-providers.json` 历史红项包含“微信 provider 未配置”；2026-07-04 当前服务端微信 provider proof 已通过，但短信实发和 `08-wechat-open-platform.png` 外部平台证据仍未归档
- `Backend/proof/ios-release-readiness.json` 历史红项包含“缺真实微信 Release build setting”；2026-07-04 本地 iOS 26.5 Release proof 已通过
- `Backend/proof/ios-app-bundle.json` 历史红项包含“缺真实 `wx...` URL Scheme”；2026-07-04 本地 iOS 26.5 包体 proof 已通过
