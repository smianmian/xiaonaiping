...
| 证据 | 文件 |
| --- | --- |
| App Store Connect 填表稿 | `Docs/08_Release/APP_STORE_CONNECT_FILL_SHEET_20260627.md` |
...
| `08-wechat-open-platform.png` | 微信开放平台移动应用配置完成 | 可见 AppID、Bundle ID、URL Scheme、Universal Link；隐藏 AppSecret、管理员账号 | 未完成 |
| `09-obs-policy.png` | 华为 OBS bucket 私有访问、加密、生命周期和删除策略 | 可见 bucket/prefix、区域、策略状态；隐藏 AK/SK 和完整对象 key | 未完成 |
| `10-final-screenshots/01-home-iphone16pro.png` | App Store 上传顺序图 1 | 不能使用真实宝宝照片、完整手机号、debug 文案 | 已有候选 |
...
6. 真机微信登录。
7. 通知权限弹窗。
8. iOS 26.5 TestFlight 或 Xcode 签名真机包回归。
9. 灵动岛、锁屏和桌面小组件视觉裁剪证据。
10. OBS 私有访问、加密、生命周期和删除策略截图。
