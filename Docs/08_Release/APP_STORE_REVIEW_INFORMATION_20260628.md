...
结构化审核测试账号执行包：`Docs/08_Release/APP_REVIEW_TEST_ACCOUNT_PACKET_20260628.json`。该包状态为 `review-test-account-packet-not-evidence`；只约束 App Review 私密 Sign-In Information 的填写、脱敏证据、RD-10/RD-13/RD-14/RD-15 真机采集、账号删除和复跑 gate，不保存恢复密钥，不能作为提交许可。
...
| 中国大陆 / App Store 人工项 | `Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260628.md`：`03-app-filing`、`04-privacy-label`、`17-age-rating-result` |
