...
- 数据方向：本地记录为主；第一版确认需要账号、自动云端同步和服务器存储照片原图，必须在后端决策、同步规则、隐私审查和区域合规中明确用途、范围、删除、加密、账号/身份识别方式。
- 商业方向：第一版免费验证，不做订阅、付费、电商或广告。
- 首发地区：全球同步首发，不采用按国家或地区分批上线。
...
- Level 3：自动云端同步和文件存储进入第一版，但不做管理后台和 BI。
- Level 4：SaaS / 多租户 / 企业权限，不进入第一版。

项目已确认需要账号、自动云端同步和照片原图服务器存储，因此后端规划必须启动；默认只保留后端部署原则，除非项目决策记录明确改用其他方案。

## 区域上线规则

首发采用全球同步上线：同一版本在计划覆盖的全部 App Store 地区同轮开放，不设置地区先后顺序或地区功能开关。

由于小奶瓶涉及儿童、照片、家庭和成长记录，全球发布前必须完成所有目标地区适用的商店、隐私、儿童数据、服务器和内容合规检查；中国大陆适用的 APP 备案、ICP 判断、个人信息保护、儿童/未成年人条款和 SDK 清单也属于同一轮发布门禁。任一目标地区的强制要求未满足时，阻断整轮全球首发，不以缩减首发地区规避。此框架不构成法律意见，发布前必须按最新规则复核。
