...
数据默认本地优先保存。用户可以在“资料 -> 账号与同步”中使用手机号验证码或微信授权登录，登录后自动同步。同步会上传宝宝记录、照片元数据，以及用户主动加入 App 的照片原图。手机号和微信登录仅用于账号识别和恢复。
...
审核认证必须与真实用户的生产手机号验证码或微信授权流程一致；正式提交包不得提供或依赖 debug code。
""".strip()
...
class ReviewNotesTest(unittest.TestCase):
    ...
    def test_incomplete_review_notes_fail(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn("metadataReviewNotesOriginalPhotos", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesDeletionPath", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesNoDebugCode", report["failedRequiredChecks"])
    ...
    def test_review_notes_require_status_display_and_advice_boundaries(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertFalse(report["passed"])
            self.assertIn("metadataReviewNotesStatusDisplayBoundary", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesNoHealthPressureFeedingAdvice", report["failedRequiredChecks"])
    ...
    def test_review_notes_require_manual_deferral_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertFalse(report["passed"])
            self.assertIn("metadataReviewNotesManualFeedingReminderDeferral", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesFeedingReminderDeferralGranularity", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesFeedingReminderDeferralOptions", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesFeedingReminderDeferralCalculation", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesFeedingReminderNoDurationFallback", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesFeedingReminderNoPersistentField", report["failedRequiredChecks"])
            self.assertIn("metadataReviewNotesNoAutomaticFeedingInference", report["failedRequiredChecks"])
