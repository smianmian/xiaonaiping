...
def write_pages(root: Path, current: bool) -> None:
    if current:
        ...
        write(
            ...
            f"第一版账号支持手机号验证码和微信授权登录。登录后自动同步。深圳市闪现生活科技有限公司。你可以手动顺延下一次喝奶提醒：可用 5 分钟一档选择不顺延或顺延 +5、+10、+15、+20、+25、+30 分钟。保存后，下一次提醒按本顿结束时间 + 固定间隔 + 顺延分钟重排；本顿无喂养时长时按本顿发生时间计算。顺延只改变下一次提醒时间，不新增持久化字段；不根据奶量、月龄、传感器或健康数据自动推算喂养时间。灵动岛、锁屏 Live Activity 和桌面/锁屏小组件只做状态展示。这些状态展示只反映用户主动记录的数据，不生成健康建议、压力提醒、喂养建议或医疗判断。不接入 HealthKit、传感器、医院系统或第三方健康数据源。资料 -> 账号与同步 -> 删除云端账号与同步。{public_urls}",
        )
        write(
    ...
    write(
        root / "Backend/static/privacy.html",
        "第一版计划首发香港和美国，不选择中国大陆，也可能先在中国大陆提交、香港为第二批。账号方式未说明。",
    )
    write(root / "Backend/static/terms.html", "第一版账号方式未说明。")
