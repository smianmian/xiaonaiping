...
4. 微信开放平台、短信服务商、OBS 策略、生产 proof 和 iOS 26.5 真机/TestFlight 证据按 `Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md` 归档。
...
已有 `Backend/proof/remote-api.json`、`Backend/proof/provider-evidence-materials.json` 和 `Backend/proof/mainland-filing-materials.json`，但提交前仍必须按 `XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md` 刷新 `20260704T-current` proof。
...
按 `CHINA_MAINLAND_LAUNCH_GAP_ASSESSMENT.md` 和 `XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md` 完成 APP 备案、正式域名、当天 `20260704T-current` 生产 proof、微信/短信/OBS、App Store 人工证据和 iOS 26.5 真机/TestFlight 验证。
""".lstrip()
...
def valid_filing_execution_packet() -> str:
    return (SCRIPT.parents[2] / "Docs/08_Release/MAINLAND_FILING_EXECUTION_PACKET_20260704.json").read_text(
        encoding="utf-8",
    )
...
def write_valid_context(
    ...
    *,
    ...
    write(
        root / "Docs/08_Release/MAINLAND_FILING_EXECUTION_PACKET_20260704.json",
...
class MainlandFilingMaterialsTest(unittest.TestCase):
    ...
    def test_stale_gap_assessment_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            write_valid_context(
                ...
                .replace("日期：2026-07-04", "日期：2026-06-18")
            ...
            self.assertIn("日期：2026-07-04", evidence)
    ...
    def test_mainland_filing_privacy_template_is_directly_validated(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn("mainlandFilingPrivacyTemplate.postCaptureChecks missing app-store-evidence-20260704T-current.json", evidence)
    ...
    def test_regional_launch_strategy_must_use_current_proof_context(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            stale = (
                ...
                .replace("日期：2026-07-04", "日期：2026-06-28")
                .replace("Backend/proof/launch-objective-audit.json", "")
                .replace("XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md", "XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260628.md")
                .replace("20260704T-current", "20260628T-current")
                .replace("不得提交", "可以提交")
            )
            ...
            self.assertIn("日期：2026-07-04", evidence)
            self.assertIn("Backend/proof/launch-objective-audit.json", evidence)
            self.assertIn("XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md", evidence)
    ...
    def test_app_store_compliance_timeline_must_use_current_gate_context(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            stale = (
                ...
                .replace("日期：2026-07-04", "日期：2026-06-18")
                .replace("Backend/proof/launch-objective-audit.json", "")
                .replace("Docs/08_Release/APP_STORE_CONNECT_DRAFT_20260704.json", "Docs/08_Release/APP_STORE_CONNECT_DRAFT.md")
                .replace("Docs/08_Release/WECHAT_RELEASE_CONFIGURATION_PACKET_20260704.json", "Docs/08_Release/WECHAT_RELEASE_CONFIGURATION_PACKET.json")
            ...
            self.assertIn("日期：2026-07-04", evidence)
            self.assertIn("Backend/proof/launch-objective-audit.json", evidence)
            self.assertIn("Docs/08_Release/APP_STORE_CONNECT_DRAFT_20260704.json", evidence)
            self.assertIn("Docs/08_Release/WECHAT_RELEASE_CONFIGURATION_PACKET_20260704.json", evidence)
    ...
    def test_china_mainland_compliance_must_use_current_proof_context(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            stale = (
                ...
                .replace("日期：2026-07-04", "日期：2026-06-28")
                .replace("Backend/proof/launch-objective-audit.json", "")
                .replace("Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md", "Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260628.md")
            ...
            self.assertIn("日期：2026-07-04", evidence)
            self.assertIn("Backend/proof/launch-objective-audit.json", evidence)
            self.assertIn("Docs/08_Release/XNP_EXTERNAL_PLATFORM_EVIDENCE_HANDOFF_20260704.md", evidence)
